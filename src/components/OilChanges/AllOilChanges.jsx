...
const OilChangeList = ({ jwt }) => {
  ...
  // Get latest oil change per car
  const latestOilChangePerCar = useMemo(() => {
    const carGroups = {};
    
    // Group by car plate
    oilChanges.forEach(change => {
      if (!carGroups[change.CarNoPlate]) {
        carGroups[change.CarNoPlate] = [];
      }
      carGroups[change.CarNoPlate].push(change);
    });
    
    // Get the latest (highest ID) oil change for each car
    const latestChanges = Object.values(carGroups).map(changes => {
  ...
  const filteredAndSortedOilChanges = useMemo(() => {
    let filtered = [...latestOilChangePerCar];
    ...
    return filtered;
  }, [latestOilChangePerCar, searchTerm, statusFilter, sortConfig]);
  ...
  const statistics = useMemo(() => {
    const critical = latestOilChangePerCar.filter(c => c.MileageLeft <= 1500).length;
    const warning = latestOilChangePerCar.filter(c => c.MileageLeft > 1500 && c.MileageLeft <= 3000).length;
    const good = latestOilChangePerCar.filter(c => c.MileageLeft > 3000).length;
    const totalCost = latestOilChangePerCar.reduce((sum, c) => sum + c.Cost, 0);
    const avgMileage = latestOilChangePerCar.length > 0 
      ? latestOilChangePerCar.reduce((sum, c) => sum + c.Mileage, 0) / latestOilChangePerCar.length 
      : 0;
    
    return { critical, warning, good, totalCost, avgMileage };
  }, [latestOilChangePerCar]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50">
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6 lg:p-8">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-blue-700 p-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                ...
                <p className="text-blue-100 mt-1">Latest oil change status for each vehicle</p>
              </div>
            ...
            <div className="bg-white rounded-xl p-4 border border-gray-200">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-800">{latestOilChangePerCar.length}</p>
                </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex gap-2">
              ...
              >
                All ({latestOilChangePerCar.length})
              </button>
              <button
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          {filteredAndSortedOilChanges.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <p className="text-gray-500 mb-8 max-w-md mx-auto">
                {searchTerm || statusFilter !== 'all' 
                  ...
                  : 'Get started by creating oil change records for your vehicles.'}
              </p>
              {(!searchTerm && statusFilter === 'all') && (
                <Link
                  to="/add-oil-change"
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filteredAndSortedOilChanges.map((oilChange) => {
                      const status = getMileageStatus(oilChange.MileageLeft);
                      ...
                      <td className="px-6 py-4 text-sm text-gray-900">
                          ...
                          <td className="px-6 py-4">
                            <div className={`inline-flex items-center px-2.5 py-1 rounded-lg text-sm font-medium ${status.color}`}>
                    ...
                    <div key={oilChange.ID} className="border-b border-gray-100 last:border-0">
                      <div className="p-4">
                        <div className="flex justify-between items-start mb-3">
                          ...
                          <div className={`px-3 py-1.5 rounded-lg text-xs font-medium flex items-center ${status.color}`}>
                            <StatusIcon size={14} className="mr-1" />
                            {oilChange.MileageLeft.toLocaleString()} km
                          </div>
                        </div>
                          ...
                          <div className="mt-3 space-y-3 bg-gray-50 rounded-xl p-4">
                            <div className="grid grid-cols-2 gap-3 text-sm">
                              <div>
                                ...
                                <p className="font-medium text-gray-900">{oilChange.Supervisor || '-'}</p>
                              </div>
                              <div>
                              ...
                              <div>
                                <p className="text-gray-500 mb-1">Current KM</p>
